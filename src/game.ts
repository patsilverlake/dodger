...
let mushroomImage: HTMLImageElement;
let bombImage: HTMLImageElement;
...
let speedBoostTimeout: number | null = null;
...
function handleTouchMove(e: TouchEvent) {
    if (!isGameRunning || isGameOver || !isTouching) return;
...
function handleTouchEnd(e: TouchEvent) {
    ...
    if (Math.abs(touchEndX - touchStartX) < touchThreshold) {
        ...
        if (tapPosition > canvasCenter) {
            ...
            setTimeout(() => { if (isGameRunning && !isGameOver) player.dx = 0; }, 300); // Stop after short time
        } else {
...
function showPowerUpMessage(message: string, color: string) {
